...
class Finetune_Physformer_MTV_GDFF(nn.Module):
    ...
    def forward(self, x, gra_sharp):
        ...
        # x_vis6 = x2_features_1.transpose(1, 2).view(b, self.dim, t//2, self.image_size//32, self.image_size//32)
        x2_features_2, Score =  self.transformer5(x2_features_1, gra_sharp)  # [B, 80*4*4, 96]  
        # x_vis7 = x2_features_2.transpose(1, 2).view(b, self.dim, t//2, self.image_size//32, self.image_size//32)

        x1_features_1, Score1 = self.transformer1(x1, gra_sharp)  # [B, 40*4*4, 96]
        # x_vis8 = x1_features_1.transpose(1, 2).view(b, self.dim, t//4, self.image_size//32, self.image_size//32)
        x1_crossfeatures_1, Score1 = self.cross_view_transformer1(x2_features_1, x1_features_1, gra_sharp)  # [B, 40*4*4, 96]
        # x_vis9 = x1_crossfeatures_1.transpose(1, 2).view(b, self.dim, t//4, self.image_size//32, self.image_size//32)
        x1_features_2, Score2 = self.transformer2(x1_crossfeatures_1, gra_sharp)  # [B, 40*4*4, 96]
        # x_vis10 = x1_features_2.transpose(1, 2).view(b, self.dim, t//4, self.image_size//32, self.image_size//32)
        x1_crossfeatures_2, Score2 = self.cross_view_transformer2(x2_features_2, x1_features_2, gra_sharp)  # [B, 40*4*4, 96]
        # x_vis11 = x1_crossfeatures_2.transpose(1, 2).view(b, self.dim, t//4, self.image_size//32, self.image_size//32)
        ...
        return rPPG, x_vis1, x_vis2, x_vis3

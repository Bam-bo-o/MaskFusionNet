...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ecg = torch.from_numpy(self.data_list[index]["ecg"]).float()
        # 重采样
        ...
        if ecg.shape[0] != video.shape[1]:
            ecg = torch.nn.functional.interpolate(ecg.view(1,1,-1),\
                 size=video.shape[1], mode="linear", align_corners=True).view(-1)
        ecg = ecg - ecg.mean()
        ecg = ecg / torch.abs(ecg).max()
        clip_hr = [cal_hr(ecg, self.transform_rate)]
        sample = {"video": video, "ecg": ecg, "clip_avg_hr": torch.tensor(clip_hr)}

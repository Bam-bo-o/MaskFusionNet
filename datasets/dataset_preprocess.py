...
class PreprocessRppgDatasets:
    ...
    def preprocess(self):
        ...
        elif self.dataset == 'BUAA':
            self.preprocess_BUAA()
        elif self.dataset == 'MMSE':
            self.preprocess_MMSE()
        else:
    ...
    def preprocess_BUAA(self):
        BUAA_sub_name = ['APH', 'GDB', 'HB', 'KLK', 'LW', 'LXR', 'LY', 'LZY', 'LMX', 'MXZ', 'PYC', 'QSH', 'WQT']
        for sub in sorted(os.listdir(self.dataset_dir)):
            sub_idx = int(sub.split(' ')[1])
            sub_path = os.path.join(self.dataset_dir, sub)
            for lux in sorted(os.listdir(sub_path)):
                ...
                pic_type = 'align_crop_pic'
                frame_dir = os.path.join(lux_path, pic_type)
                frame_path = []
                frame_list = os.listdir(frame_dir)
                frame_list_int = [int(i.split(".")[0]) for i in frame_list]
                frame_list_int.sort()
                ...
                # read label
                wave_csv = os.path.join(lux_path, f"lux{lux_rate}_{BUAA_sub_name[sub_idx-1]}_wave.csv")
                with open(wave_csv, 'r') as f:
                    data = f.readlines()
                    data = data[1:]
                    ecg = np.array([int(i) for i in data])
                ...
                sample["video_data"] = video_x.transpose(3, 0, 1, 2) # (C, T, H, W)
                sample["ecg_data"] = ecg_signals
                print(f'\rsaving {lux_path}/sample.hdf5', end='')
                with h5py.File(f'{lux_path}/sample.hdf5', 'w') as f:
                    for key in sample.keys():
                        f.create_dataset(key, data=sample[key])

    def preprocess_MMSE(self):
        with open(f'{self.dataset_dir}/white_name.txt', 'r') as f:
        ...
        for subject in subject_list:
            ...
            for source in source_list:
                if os.path.join(subject_path, source) not in white_name_list:
                    continue
                pic_type = 'pic'
                frame_dir = os.path.join(subject_path, source, pic_type)
                frame_path = []
                frame_list = os.listdir(frame_dir)
                frame_list_int = [int(i.split(".")[0]) for i in frame_list]
                frame_list_int.sort()
                for frame_name in frame_list_int:
                    frame_bit = len(frame_list[0].split(".")[0])
                    frame_path.append(os.path.join(frame_dir, f"{frame_name:0>{frame_bit}}.png"))
                # read label
                wave_csv = os.path.join(subject_path, source, "Pulse Rate_BPM.txt")
                with open(wave_csv, 'r') as f:
                    data = f.readlines()
                    ecg = np.array([float(i) for i in data])
                
                video_x = self.read_video(frame_path, cur_fps=30, target_fps=30)
                ecg_signals = self.resample_ecg(ecg, target_len=len(video_x)).reshape(-1)

                sample = {}
                sample["video_data"] = video_x.transpose(3, 0, 1, 2) # (C, T, H, W)
                sample["ecg_data"] = ecg_signals
                sample["gt_hr"] = ecg_signals.mean()
                print(f'saving {subject_path}/{source}/sample.hdf5')
                with h5py.File(f'{subject_path}/{source}/sample.hdf5', 'w') as f:
                    for key in sample.keys():
                        f.create_dataset(key, data=sample[key])
...
if __name__ == '__main__':
    ...
    args.add_argument('--dataset', type=str, default='VIPL')
    args.add_argument('--dataset_dir', type=str, default='/mnt/data/VIPL')
